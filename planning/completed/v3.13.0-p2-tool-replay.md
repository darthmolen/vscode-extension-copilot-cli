...
(§5.1), and replayed tool results are truncated rather than carried whole (§5.4).
...
export interface Message {
    ...
    /** Present only when kind === 'tool'. The shape `buildToolHtml` actually reads — see below. */
    tool?: ToolState;
    agentId?: string;
}
```
...
#### `ToolState` must be corrected first — it describes nothing that exists

`src/shared/models.ts:41-50` declares `ToolState` as `{ id, name, input, output, error: string }`.
`buildToolHtml` reads `toolCallId`, `toolName`, `arguments`, `result`, `error: { message, code }`.
The identifying and payload fields all diverge and `error` differs in *shape*, so a `tool?: ToolState`
imported from `shared/` would hand the renderer `undefined` for every field — an empty chip, the
original bug reproduced, with **no type error** because the webview is untyped JS.

The type that actually flows at runtime is `ToolExecutionState` (`sdkSessionManager.ts:225-238`).
`ExtensionRpcRouter.toolStart(toolState: ToolState)` declares the shared type while
`chatViewProvider.addToolExecution(toolState: any)` passes `any`, so the declaration is never checked.
**The wire type is already wrong today, on the live path** — replay merely makes it fatal.

**Fix: align `models.ts:ToolState`'s fields to the runtime shape** (`toolCallId`, `toolName`,
`arguments`, `result`, `error: { message, code }`, `startTime`, `endTime?`, `intent?`, `progress?`,
keeping `status`, `agentId`, `parentToolCallId`, which already match). Nothing reads the old names —
the webview already reads `toolCallId`/`toolName` — so this is a declaration-only change that turns a
trap into a contract.
...
### 5.2 One projection, tool-aware — `sessionTranscriptBuilder`

**Home:** `src/extension/services/sessionTranscriptBuilder.ts`, exporting
`buildSessionTranscript(eventsPath: string, options?): Promise<Message[]>`.

Named for what it **builds**, following `mcpStatusBuilder.ts` — the house pattern for a lowercase
module of free functions, as distinct from the PascalCase `*Service.ts` files, which are classes.
Not `SessionEventReader`: you do not get events back, and reading is not where the value is. It joins
`start`/`complete`, derives status from a boolean and reconstructs a tool state — a **projection** of
an append-only log into a read model, which is what the thing actually is.

A free function, not a method, because **ACP's `session/load` is a third caller in another process**
(§8a). It also stays free of policy: deciding *whether* to resume or attach is
`ChatSessionHost.ensureStarted()`'s job (Task 6). Keeping the fold pure is what keeps it reusable.

`SessionService.loadSessionHistory` — today's tool-dropping reader — is **deleted**, not left as a
delegate. Two history paths is how §4's divergence happened.
...
| `complete.data.error` | `tool.error` — `{ message, code }`, present on failures |
| `complete.data.result` | `tool.result`, **truncated to a cap and marked** — §5.4 |
...
**Not set:** `hasDiff` — the *View Diff* button points at
...
**And replay must not inherit stale live tool state.** `MessageDisplay.clear()` calls
`this.toolExecution.clear()` guarded by `typeof … === 'function'` — and **`ToolExecution` has no
`clear()`**, so the guard is dead and `tools`, `currentToolGroup` and `collapsedCards` survive
re-init. With `retainContextWhenHidden: true`, the sidebar hide/show path — precisely the one that
produces the "Tool execution" wall — re-inits a webview whose `ToolExecution` still points at DOM that
`clear()` just wiped. **`ToolExecution` gains a real `clear()`**, which makes that guard live. This is
a latent bug in today's code; replay is where it surfaces.

### 5.4 Results are truncated, not lazily fetched

`data.result` is unbounded — a `bash` that printed a build log, a `view` of a large file — and 48 of
them would ship on every rehydration, including the sidebar hide/show path where nothing on screen
changed.

**`buildSessionTranscript` truncates each result to a cap and marks it truncated.** One line, bounded
payload, and real output for the great majority of tools, whose output is small.

The richer version — carry nothing, fetch a result when its chip is expanded — is designed and
**deferred** to `planning/backlog/lazy-tool-results-on-rehydration.md`. It costs six touch points and
a new RPC round-trip to avoid carrying strings, and the bug under repair is *"replayed tools render as
a wall of Tool execution"*, which truncation fixes just as completely. Nothing here forecloses it:
`toolCallId` is in the log either way.
...
5. **Truncation test**: a long result is capped and marked; a short one is carried whole.
6. **`ToolExecution.clear()` exists and resets `tools` / `currentToolGroup` / `collapsedCards`**, and
   `MessageDisplay.clear()`'s guard is no longer dead — asserted, because the sidebar hide/show path
   depends on it.
7. **`role` alias retirement is *not* part of this work.** It stays for the whole v3.13.0 release, and
   the task that removes it must first change `ToolExecution.js:35-43`, which closes tool groups on
   `message.role === 'assistant'` in the **live** path. Recorded here so it is not lost.
8. Suite / `check-types` / `lint` / `node esbuild.js` green. **The suite is flaky in a documented way
   — a failure counts only if it also fails when its file is run alone** (`npx mocha <file>
   --timeout 20000`).
9. **Manual, in the Extension Development Host:** reload the sidebar on a session with tool calls —
   real tool names, real final statuses, no "Tool execution" wall; expand a chip and see its
   (truncated) output; switch away and back and get the same transcript.
