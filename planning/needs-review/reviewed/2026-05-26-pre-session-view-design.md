...
**Reviewed:** 2026-06-18 17:33
**Reviewer:** Claude Code (plan-review-intake)

### Prior Review Issue Status

**Critical 1 — Component ownership conflict (Sections 7–9/8):** STILL PRESENT
- Plan says main.js mounts PreSessionView into a "slot in the messages container header" — but MessageDisplay.clear() (lines 737–754) wipes all  elements. The plan doesn't specify how PreSessionView survives clear() or what this "slot" is.

**Critical 2 — Wrong startup model (Section 7 / Testing):** STILL PRESENT
- Manual verification step 4 still says "click 'Start a chat session'" — but extension.ts:186–188 shows  auto-starts. No click is required.
-  (main.js:601–616) replays historical messages via  — first historical message would trigger collapse before user ever types.

**Important 1 — BufferedEmitter doesn't satisfy webview recreation:** STILL PRESENT
- BufferedEmitter.js (lines 35–41) flushes and clears the buffer on first listener. A recreated webview would get nothing. BackendState has no system-status snapshot field. Plan doesn't address this.

**Important 2 — Slow-path event contract under-specified:** STILL PRESENT
- Plan doesn't specify what completes  when moving to . Does resolving fire  before installing fires ?

**Important 3 — Test paths don't match repo conventions:** STILL PRESENT
- Plan proposes  — no such directory exists. Correct path is .

**Important 4 — Manual verification inaccurate:** STILL PRESENT
- Step 1 still doesn't specify the actual managed cache path (CliBundleService uses , not ). Step 4 still says "click start session."

**Important 5 — Subscription lifecycle unspecified:** STILL PRESENT
- No disposal or dedup logic described for repeated ready/reload cycles.

### New Issues Found

#### Critical

**CliBundleService constructor injection breaks existing callsites**
- Section: Section 2 (backend)
- What's wrong: Plan shows emitter as a 4th positional parameter — . This would break all existing callsites. The  callback belongs inside , not as a new positional argument.
- Why it matters: This is a compile-breaking change that affects extension.ts and any test that constructs the service.
- Fix: Add  to .

**SDKSessionManager.start() fires session.ready on every restart, not just first run**
- Section: Section 4
- What's wrong:  is also called by  (line 1760–1762). Emitting  here means every restart would show "Session started in Ns" as if it's a fresh startup.
- Why it matters: The UI would behave confusingly on restart — re-showing startup phases that aren't relevant.
- Fix: Gate emission on an "initial startup only" flag, or use a distinct event for restart vs first-start.

**PreSessionView never stops subscribing after collapse**
- Section: Section 7
- What's wrong: Plan says collapse on first , but the component keeps listening to  forever. No cleanup or deactivation lifecycle defined.
- Why it matters: Subsequent CLI re-install events would hit an already-collapsed component with undefined behavior.

**Version number is wrong**
- Section: Versioning
- What's wrong: Plan says bump to 3.9.0 but current version in package.json is 3.11.0. Should be 3.12.0.
- Why it matters: An implementer following the plan literally would downgrade the version number.

#### Important

**No mechanism to reset phase state between sessions**
- BackendState.reset() doesn't clear system-status state (the plan never added it). Fresh sessions after restart would show stale phase data or nothing.

**RPC sender method not wired in Section 6**
- Section 9 adds  to WebviewRpcClient, but Section 6 (ExtensionRpcRouter) only says "add ". The plan doesn't show the complete send→receive wiring, making Phase 9 ambiguous.

**No error recovery if PreSessionView.js fails to load**
- esbuild.js copy is flagged as CRITICAL but no fallback is defined. If the file is missing, the sidebar goes blank with no explanation.

#### Minor

**Icon choices are emoji, not Codicons**
- , ,  render differently on macOS/Windows/Linux. VS Code Codicons (, , ) would be consistent across platforms.

**"1-2 min" install time estimate is unsupported**
- No data to justify the range. Could be 30s or 5min depending on network. Should say "this may take a few minutes" or be omitted.

**No accessibility spec**
- Spinner needs ; phase transitions need ; collapsed summary needs keyboard interaction support.

### Assessment

**Implementable as written?** No

**Reasoning:** All five critical/important issues from the prior review remain unaddressed, plus the plan contains a compile-breaking constructor change and a wrong version number. The core mental model also mismatches reality — the plan treats session start as user-initiated when it's actually automatic resume. Needs a ground-up revision around the actual  flow, with phase state persisted in BackendState and PreSessionView living outside MessageDisplay's DOM.
