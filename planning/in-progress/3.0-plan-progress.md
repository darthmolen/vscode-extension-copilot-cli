...
**Overall Status: ✅ Phase 0 & 0.1 Complete (2026-02-09)**

| Phase | Service/Work | Status | Lines Saved | Tests Added | Notes |
|-------|--------------|--------|-------------|-------------|-------|
| **Phase 0** | **PlanModeToolsService Enhancement** | ✅ **VERIFIED** | **-119 lines** | **9 tests** | **Configuration abstraction tested & working in production** |
| **Phase 0.1** | **Critical Bug Fixes** | ✅ **COMPLETE** | **N/A** | **7 tests** | **Fixed filediff crash & View Plan button visibility** |
| Phase 1 | ConfigurationService | ⏳ Not Started | Target: -50 | Target: 6 | - |
| Phase 2 | SessionService | ⏳ Not Started | Target: -200 | Target: 12 | - |
| Phase 3 | StartupService | ⏳ Not Started | Target: -140 | Target: 8 | - |
| Phase 4 | CommandHandlerService | ⏳ Not Started | Target: -200 | Target: 12 | - |
| Phase 5 | ActiveFileTrackerService | ⏳ Optional | Target: -40 | Target: 7 | - |
| Phase 6 | Final Refactor - Organize All | ⏳ Not Started | N/A | N/A | Move all 10 services to `src/extension/services/` |

**Current Metrics:**
- sdkSessionManager.ts: ~~1568~~ → **1449 lines** (-119, target: ~1450) ✅ **TARGET MET!**
- extension.ts: Still 879 lines (target: ~200)
- chatViewProvider.ts: Still 539 lines (target: ~300)
- New tests: **16 total** (9 config + 7 bug fixes, target: 54+)
...
This ensures:
- Established TDD workflows are followed (RED → GREEN → REFACTOR)
...
## Status
🟢 ~70% Complete (5 services extracted in v2.2+)
...
**Target Metrics:**
- extension.ts: 879 lines → ~200 lines (77% reduction) 🎯
- sdkSessionManager.ts: 1568 lines → ~1450 lines (7.5% reduction) 🎯
- chatViewProvider.ts: 539 lines → ~300 lines (44% reduction) 🎯
- All services: 100% test coverage ✅
- All services: Organized in `src/extension/services/` ✅

## Completed Services ✅ (v2.2+)
...
### 0. Enhance PlanModeToolsService 🎯 HIGHEST PRIORITY (Configuration Abstraction)
...
**Why enhance?**
- ✅ Removes 118 lines of hard-coding from sdkSessionManager
- ✅ PlanModeToolsService becomes single source of truth for plan mode tool knowledge
- ✅ Enables future UI customization of tools/prompts
- ✅ Enables system prompt engineering/testing
- ✅ Configuration abstraction pattern for other services
...
12. REFACTOR: Run all tests → verify no regressions
13. INTEGRATION: Build and test with `./test-extension.sh`

**Tests to write** (plan-mode-tools-service-configuration.test.js):
- getAvailableToolNames() returns array of 11 tool names ✓
- getAvailableToolNames() includes all custom tools (6) ✓
- getAvailableToolNames() includes all SDK tools (5) ✓
- getAvailableToolNames() includes report_intent ✓
- getSystemPrompt() generates prompt with plan path ✓
- getSystemPrompt() includes correct session ID in path ✓
- getSystemPrompt() includes all tool descriptions ✓
- getSystemPrompt() includes workflow instructions ✓
- getSystemPrompt() includes restrictions ✓

**Benefits**:
- ✅ Configuration abstraction (tools and prompts configurable)
- ✅ Future UI for customization (tool selection, prompt editing)
- ✅ System prompt engineering/testing enabled
- ✅ sdkSessionManager becomes pure orchestration
- ✅ PlanModeToolsService is single source of truth

---

### 1. SessionService 🎯 HIGH PRIORITY
...
- Determine session to resume with folder filtering ✓
- Determine session to resume without folder filtering ✓
- Handle no workspace open ✓
- Format session label with plan.md title ✓
- Format session label without plan.md (fallback to ID) ✓
- Load session history from events.jsonl ✓
- Handle missing events.jsonl ✓
- Parse user.message and assistant.message events ✓
- Skip malformed JSON lines ✓
- List all sessions with metadata ✓
- Filter sessions by workspace folder ✓
- Get most recent session ✓

---

### 2. ConfigurationService 🔥 MEDIUM PRIORITY
...
- Get CLI config with YOLO mode enabled (overrides all) ✓
- Get CLI config with YOLO mode disabled ✓
- Get individual config values ✓
- Handle missing config (defaults) ✓
- Configuration change event fires ✓
- YOLO overrides allowAllTools, allowAllPaths, allowAllUrls ✓

---

### 3. StartupService 🔥 MEDIUM PRIORITY  
...
- Resume last session successfully ✓
- Resume specific session by ID ✓
- Handle auth error - no env var (Scenario 1) ✓
- Handle auth error - invalid env var (Scenario 2) ✓
...
### 4. CommandHandlerService 💡 MEDIUM PRIORITY
**Lines to extract**: ~200 from extension.ts
...
- Open chat command ✓
- Start chat command ✓
- New session command (stops existing, starts new) ✓
- Switch session command ✓
- Stop chat command ✓
- Refresh panel command ✓
- View diff command ✓
- Toggle plan mode command ✓
- Accept plan command ✓
- Reject plan command ✓
- Error handling in each command ✓

---

### 5. ActiveFileTrackerService 💡 LOW PRIORITY (Optional)
...
- Respect includeActiveFile setting (enabled) ✓
- Respect includeActiveFile setting (disabled) ✓
- Keep last known file when focus moves to webview ✓
- Clear when all editors close ✓
...
4. Run tests → **VERIFY THEY FAIL** (module not found)
...
8. Run **ALL** tests → **NO REGRESSIONS**
...
### Phase 0: Enhance PlanModeToolsService (Configuration Abstraction) 🎯 ✅ COMPLETE
- [x] RED: Create `tests/plan-mode-tools-service-configuration.test.js`
- [x] RED: Write 9 test cases (tool names, prompt generation)
- [x] RED: Run tests → verify failures (9 failing tests confirmed)
...
- [x] GREEN: Run tests → verify passing (all 9 tests pass)
...
- [x] REFACTOR: Run all tests → verify no regressions (12/12 integration tests pass)
- [x] Integration: Build and test with `./test-extension.sh` (successful)
- [x] Verify plan mode still works (toggle, create plan, accept/reject) - ✅ **USER VERIFIED (2026-02-09)**
...
- ✅ **Code Reduction Achieved**
  - sdkSessionManager.ts: 1568 → 1449 lines (**-119 lines, -7.6% reduction**)
...
- ✅ **Production Verified (2026-02-09)**
...
**Two critical bugs discovered and fixed after Phase 0 deployment:**
...
**Root Cause**: Code assumed `payload.data.toolCallId` always exists, but sometimes receives direct payload without `.data` wrapper  
...
**Problem**: View Plan button always hidden, even when workspace is open  
**Location**: `src/webview/main.js:886` (`handleInitMessage`)  
**Root Cause**: `handleInitMessage()` function didn't process `payload.workspacePath` from init message  
...
- [x] **Bug 1: filediff Tool**
  ...
  - [x] Commit: `be7457a` - "fix: Handle diffAvailable payload without .data wrapper"

- [x] **Bug 2: View Plan Button**
  ...
  - [x] Commit: `2bf31db` - "fix: Show View Plan button when workspace is available"

- [x] **Cleanup**
  - [x] Fix directory naming: removed space from `planning/ in-progress/`
  - [x] Commit: `04c7b5b` - "fix: Remove space from directory name"

**Phase 0.1 Summary - Completed 2026-02-09:**

- ✅ **Two Critical Bugs Fixed**
  - filediff tool: Crash on undefined payload.data (defensive coding applied)
  - View Plan button: Always hidden due to missing workspacePath initialization
- ✅ **TDD Approach Followed**
  - Created tests first to document expected behavior
  - Applied minimal fixes to resolve issues
  - 7 new unit tests added (3 + 4)
- ✅ **Code Quality**
  - Both fixes use defensive coding patterns
  - Support multiple payload formats for resilience
  - No breaking changes to existing functionality
- ✅ **Git Hygiene**
  - Each bug fix committed separately
  - Clear commit messages with root cause analysis
  - 3 commits: 2 bug fixes + 1 cleanup
- ⏳ **Pending**
  - Manual testing of both fixes (awaiting user verification)
  - Push to remote after verification

**Impact**: Fixes two user-facing bugs that would prevent proper use of filediff and plan viewing features.
...
- [ ] GREEN: Run tests → verify passing
- [ ] REFACTOR: Replace all scattered config reads
- [ ] REFACTOR: Run all tests → verify no regressions
...
### Phase 6: Final Refactor - Organize All Services 🎯
**Goal**: Move all services (existing + new) to `src/extension/services/` directory

**Services to move**:
1. `src/modelCapabilitiesService.ts` → `src/extension/services/ModelCapabilitiesService.ts`
2. `src/mcpConfigurationService.ts` → `src/extension/services/MCPConfigurationService.ts`
3. `src/planModeToolsService.ts` → `src/extension/services/PlanModeToolsService.ts`
4. `src/fileSnapshotService.ts` → `src/extension/services/FileSnapshotService.ts`
5. `src/messageEnhancementService.ts` → `src/extension/services/MessageEnhancementService.ts`
...
- [ ] Build: `npm run compile` → verify no errors
- [ ] Run all tests: `npm test` → verify all pass
...
- [ ] Update CHANGELOG.md
- [ ] Update phase-3-extract-services.md to mark ✅ COMPLETE
...
**Core Functionality**:
- [ ] Open chat panel (Ctrl+Shift+P → "Copilot CLI: Open Chat")
...
**Code Reduction**:
- ✅ extension.ts: 879 → ~200 lines (77% reduction) 🎯
- ✅ sdkSessionManager.ts: 1568 → ~1450 lines (-118 lines, -7.5%) 🎯
- ✅ chatViewProvider.ts: 539 → ~300 lines (44% reduction) 🎯
- ✅ All new services under 200 lines each

**Organization**:
- ✅ All 10 services in `src/extension/services/` directory
- ✅ Clean barrel export (optional)
- ✅ Consistent naming: PascalCase service files

**Configuration Abstraction** (NEW!):
- ✅ PlanModeToolsService owns ALL plan mode configuration
- ✅ No hard-coded arrays in sdkSessionManager
- ✅ No hard-coded prompts in sdkSessionManager
- ✅ Future-ready for UI customization
- ✅ System prompt engineering enabled
...
- [ ] Configuration abstracted from orchestration ✨ NEW
...
- [ ] All services organized in proper directory ✅
- [ ] No hard-coded configuration ✨ NEW
...
- ❌ Do NOT implement MCP UI yet (that's Phase 6)
- ❌ Do NOT extract services speculatively (only extract what we need now)
- ❌ Do NOT create full PlanModeService (enhance existing PlanModeToolsService instead)

---

## Dependencies
- ✅ Phase 2 (RPC layer) is complete
- ✅ 5 services already extracted in v2.2+
...
This is the power of configuration abstraction! 🚀
