...
export class ChatPanelProvider implements vscode.Disposable {
	...
	private viewStateChangeCount = 0;
	private lastViewStateChange = 0;
	private updateSessionsListDebounceTimer: NodeJS.Timeout | undefined;
	private readonly UPDATE_SESSIONS_DEBOUNCE_MS = 500; // Wait 500ms before updating
	private lastActiveState: boolean = false;
	private lastVisibleState: boolean = true;
	...
	public createOrShow() {
		...
		this._reg(this.panel.onDidChangeViewState(
			e => {
				// Track for loop detection
				const now = Date.now();
				const timeSinceLastChange = now - this.lastViewStateChange;
				this.viewStateChangeCount++;
				this.lastViewStateChange = now;
				
				// WARN if firing too frequently (potential loop)
				if (timeSinceLastChange < 100) {
					this.logger.warn(`⚠️ [FOCUS LOOP?] viewState changed ${this.viewStateChangeCount} times, ${timeSinceLastChange}ms since last`);
				}
				
				// Detect what actually changed
				const activeChanged = this.lastActiveState !== e.webviewPanel.active;
				const visibleChanged = this.lastVisibleState !== e.webviewPanel.visible;
				const viewColumnChanged = this.lastViewColumn !== e.webviewPanel.viewColumn;
				...
				if (activeChanged && !visibleChanged && !viewColumnChanged) {
					this.lastActiveState = e.webviewPanel.active;
				...
				this.logger.debug(`[ViewState] Change #${this.viewStateChangeCount}: active=${e.webviewPanel.active}, visible=${e.webviewPanel.visible}, column=${e.webviewPanel.viewColumn}`);
				
				// Log meaningful changes at INFO level (one line per change)
				if (viewColumnChanged) {
					this.logger.info(`[Panel Move] Column ${this.lastViewColumn} → ${e.webviewPanel.viewColumn}`);
					this.lastViewColumn = e.webviewPanel.viewColumn;
				}
				
				if (visibleChanged) {
					this.logger.info(`[Visibility] Panel ${e.webviewPanel.visible ? 'shown' : 'hidden'}`);
					this.lastVisibleState = e.webviewPanel.visible;
				}
		...
		this.rpcRouter = new ExtensionRpcRouter(this.panel.webview);
	...
	public addUserMessage(text: string, attachments?: Array<{displayName: string; webviewUri?: string}>, storeInBackend: boolean = true) {
		...
		this.rpcRouter?.addUserMessage(text, attachments as any); // Type compat: different Attachment shapes
	}
	...
	public updateSessions(sessions: Array<{id: string, label: string}>, currentSessionId: string | null) {
		...
		this.rpcRouter?.updateSessions(sessions as any, currentSessionId); // Type compat: different Session shapes
	}
