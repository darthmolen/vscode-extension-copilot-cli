...
- **Plan mode keeps its conversation** — Entering plan mode a second time used to start the planning discussion over: the agent had no memory of what you had just planned. The plan session is now resumed rather than re-created, so the conversation carries across re-entry. Plan mode's tool restrictions (no write, commit, or install) are enforced on the resume path too — both verified by spike, see `planning/spikes/plan-session-reuse/`.
- **Plan mode survives a restart** — Close VS Code while planning and you come back in plan mode, with the planning conversation intact. Two things blocked this. Entering plan mode was never *recorded* as your choice, so the next launch restored the work half and silently dropped planning; and startup resurrected the work session before restoring plan mode, which failed outright when that work session had no transcript (see the fix below). Startup now resumes the plan session directly and defers the work session to the moment you leave plan mode — which is the only moment it is needed, and where the code already created one on demand.

### 🐛 Bug Fixes

- **Chat no longer loads another project's conversation** — Opening VS Code in a folder with no prior session fell back to the globally most-recent session and loaded it, so you could open project A and be shown project B's transcript, session id, and CLI session. Selection now returns nothing when no session belongs to the current folder, and a fresh session starts instead. This is what `copilotCLI.filterSessionsByFolder` always promised; setting it to `false` still opts out into the global behaviour.
- **Windows drive-letter casing no longer breaks folder matching** — Session paths are recorded inconsistently (lowercase drive in `cwd`, uppercase in `git_root`, within one file) and VS Code's own casing varies. Path comparison is now case-insensitive on Windows, so sessions stop being silently excluded from their own folder. POSIX comparison is unchanged.
- **Sessions with a large first event are no longer dropped** — A session's working directory was read from the first 2KB of `events.jsonl`; a longer first line truncated, failed to parse, and the session was treated as belonging to no folder. `workspace.yaml` is now the primary source, with `events.jsonl` as fallback.
- **"Previous session not found" on startup** — Enter plan mode without first sending a work-mode message and the work session is created but never gets a transcript, so the CLI cannot resume it. The recorded-session path accepted it anyway, because it checked only that the session *directory* existed — a weaker test than the one the rest of session selection uses. Session selection now asks whether a session can be *restored*, which is a different question for each half: a work session needs a transcript (bringing it back means `session.resume`), a plan session does not (bringing it back means re-entering plan mode, which creates the plan session when there is none).
- **The model dropdown no longer reports a model you are not using** — When the configured model does not exist for your account, the extension falls back (to `auto`) and the session really does run on it. But the UI read `copilotCLI.model` — the *setting* — so it kept showing the dead model, and `supportsVision()` / `getMaxImages()` asked about a model absent from the catalogue. The CLI names the model it actually selected in `session.start`; that is now adopted as the truth, and a `model_switched` event moves the dropdown with it. A stale `claude-sonnet-4.6` entry in the selector's static price catalogue — which is why a nonexistent model still got a confident cost badge — is gone.
- **Tool groups close again** — Tools piled into a single accordion for a whole session. Closing was driven only by an assistant message carrying visible content, which held while the model narrated in separate messages — a habit it had while `report_intent` existed, because it discharged narration into that tool call. Without it the model narrates inline with its tool requests, and that content is suppressed as a mid-thought fragment (ADR-006 Decision 3), so nothing carried content and nothing closed. Measured on a real run: 41 turn ends against 2 content-bearing messages. Groups now close on `assistant.turn_end`, which a tool group actually belongs to and which does not depend on how the model distributes its prose.
- **The tool line has its label back** — Tool rows were annotated with what the agent was doing, sourced from a `report_intent` tool the model called. CLI 1.0.80 does not have that tool, so the label went bare. The CLI now supplies it directly: each entry in `assistant.message.toolRequests` carries `intentionSummary`, keyed by the same `toolCallId` that `tool.execution_start` reports. Better than before — the old code held a single most-recent intent and handed it to whichever tool started next, so a message requesting three tools labelled one and left two bare, and any mismatch mislabelled silently.
- **Plan mode no longer claims two tools that do not exist** — The plan-mode whitelist named `report_intent` and `fetch_copilot_cli_documentation`, and CLI 1.0.80 answers `Unknown tool name in the tool allowlist` for both. Neither was ever one of our custom tools; both were assumed to be CLI built-ins. Worse than the warning: both were also advertised to the model in the plan-mode system prompt, so it was told it had tools it could never call. Removed from the whitelist and the prompt together.
- **Replayed transcripts no longer stitch separate runs together** — A session id that has been re-created keeps its earlier lines in the same `events.jsonl`. The transcript replayed all of them as one conversation, even though the agent only remembered the last run. `buildSessionTranscript` now returns only the messages after the most recent `session.start`.

### 🔧 Internal

- **Skill directories read from `installed_plugins.json`** — Plugin skill directories were found by walking `~/.claude/plugins/cache/**`, which retains every version ever fetched plus uninstalled leftovers. On one machine that passed a stale plugin version alongside the installed one, colliding on 17 skill names, plus a plugin that was not installed at all. Resolution now reads Claude Code's manifest, which names exactly one path per installed plugin, falling back to the directory walk when the manifest is missing or an unrecognised schema. Note: spikes could not reproduce a user-visible failure from the old behaviour — see `planning/spikes/skill-discovery/FINDINGS.md`. This is a correctness fix, not a confirmed cure for missing skills.
- **Skill resolution is logged** — Resolved directory count at INFO, each path at DEBUG, once per session. Previously silent, which is why a wrong list could go unnoticed.
- **`resolveStartupPairing()`** (new, in `sessionPairing.ts`) — Answers work-or-plan for the single session startup is about to resume. `sdkSessionManager` matched `-plan` with its own `endsWith` call, which is the uncoordinated reader `sessionPairing` was written to eliminate; the convention is back to one place. Exempt from that module's batch-only rule because it is called once, not in a loop — the guard test now records why.
- **`collectToolIntents()`** (new, exported from `sdkSessionManager.ts`) — Maps `toolCallId` to `intentionSummary` from a message's tool requests. Replaces the single `lastMessageIntent` slot.
- **`resolveActiveModel()`** (new, exported from `sdkSessionManager.ts`) — Decides when to adopt the model the CLI reports over the one being tracked. Ignores a missing or non-string value rather than blanking the display.
- **`SessionService.isRestorable()`** (new) — Whether a recorded session id can be brought back, which is not the same as whether it can be resumed. Transcript required for a work session; pairing sufficient for a plan session.
- **`SessionService.hasSessionHistory()`** (new) — Decides whether a session id can be resumed or must be created. Backs the plan-mode resume path.
- **`src/sessionUtils.ts` deleted** — Dead duplicate of the session-selection logic carrying its own copy of the global-fallback bug. Nothing in `src/` imported it; its only consumer was an integration test, now migrated to `SessionService`.
- **`plan-mode-tools-service-configuration.test.js` now imports the real service** — It defined its own `PlanModeToolsService`, a "GREEN phase: Minimal implementation" that was never replaced, duplicating the tool list and system prompt so every assertion checked a copy against itself. That is why it never noticed the two nonexistent tools. Pointing it at production code also fixed one of the suite's standing Windows failures, which was a POSIX path baked into the copy.
- **Test isolation fix** — `plan-mode-tools-service-di.test.js` stubbed `os.homedir` without restoring it, leaking a deleted temp path into every later test file in the same mocha run.
...
- **No more `cmd.exe` console window on session start (Windows)** — When the CLI was spawned via `npm-loader.js`, that loader internally `spawnSync`'d the native binary without `windowsHide: true`, producing a persistent console window for the session lifetime; closing it killed the session. The extension now uses a hybrid spawn strategy (see next bullet): when system Node 24+ is available, the SDK spawns the pure-Node `index.js` entrypoint under it; otherwise `cliPath` points directly at the native CLI binary (`@github/copilot-${platform}-${arch}/copilot[.exe]`). Neither path goes through `npm-loader.js`'s inner `spawnSync`, so the SDK's outer `windowsHide: true` is respected end-to-end — no console window at any point.
