...
describe('main.js Message Handler Regressions', () => {
	...
	describe('InputArea method compatibility', () => {
		...
		it('updateFocusFile() should update ActiveFileDisplay child', () => {
			inputArea.updateFocusFile('/workspace/src/test.ts');

			const activeFileDisplay = inputArea.container.querySelector('.active-file-display');
			expect(activeFileDisplay, 'ActiveFileDisplay should render').to.exist;
			// Should be visible when file is set
			expect(activeFileDisplay.style.display).to.not.equal('none');
		});
