...
> **Note**: This document was originally written during the SDK 0.3.0 migration. The extension now uses SDK ^1.0.5. The bundling architecture described here remains in place.

> **TL;DR**: The current SDK (`^1.0.5`) still requires a compatible Copilot CLI JSON-RPC permission protocol. We don't trust the user's global `copilot` install anymore — the extension owns its CLI version via lazy install into `globalStorage`, with the version pinned by the SDK's own `dependencies`/`peerDependencies` declaration. This document is the post-mortem and the runbook.

## What Broke (The Symptom)

During the migration path that led to the current `@github/copilot-sdk` dependency (`^1.0.5`), every tool call failed:
...
1. **SDK ^1.0.5** declares a dependency on `@github/copilot@^1.0.67`. The JSON-RPC protocol between SDK and CLI changed in that range — both sides must be in sync.
...
Even though `node_modules/@github/copilot` locally installed 1.0.67 (matching the SDK), the **installed VSIX had `node_modules/**` stripped** by `.vscodeignore`, so the running extension had nothing to spawn except the system `copilot` binary on PATH — which was 1.0.5.
...
| Approach | Outcome |
|----------|---------|
| Revert the SDK change from `^1.0.5` | Hides the problem; SDK ↔ CLI version skew will bite us again on the next upgrade |
...
return pkg.dependencies['@github/copilot'];   // "^1.0.67"
```
...
**Subtle catch on the same pass**: SDK ^1.0.5's `package.json` puts `@github/copilot` in `dependencies`, not `peerDependencies` as we'd assumed. Both reads (build and runtime) now check `peerDependencies` first then fall back to `dependencies`.
...
```text
[CLI Bundle] source=local version=1.0.67 satisfies=true peerRange=^1.0.67
[CLI Bundle] source=managed version=1.0.67 satisfies=true peerRange=^1.0.67
[CLI Bundle] source=system version=1.0.5 satisfies=false peerRange=^1.0.67
```
...
```text
## Copilot CLI
CLI version: **1.0.67** (local, satisfies ^1.0.67)
```
